...
('MHSPT25',N'NGUYỄN CẨM LAN','ML12K','5/9/2000',N'TÂN PHÚ,HCM',N'NỮ','MKHPT14')
...
alter table KETQUA
ADD CONSTRAINT CHECK_K_Q CHECK ((DIEM >= 0) OR (DIEM <= 10));

CREATE TRIGGER TINH_KT ON KETQUA
FOR INSERT, UPDATE
AS
	UPDATE KETQUA
	SET DIEMTK = (DIEMHK1 + (DIEMHK2*2))/3
	WHERE KETQUA.MAHS = (SELECT MAHS FROM INSERTED WHERE INSERTED.MAHS = KETQUA.MAHS)
GO


select MAHOCSINH, TENHOCSINH, GIOITINH, NGAYSINH, DIEMTK
 from LOP, HOCSINH, KETQUA
WHERE HOCSINH.MALOP = LOP.MALOP AND TENLOP = N'6A'
and KETQUA.MAHS = MAHOCSINH
ORDER BY DIEMTK ASC
